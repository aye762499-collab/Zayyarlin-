...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'zayyarlin_tt_pro_v3')

# --- ဇေယျာလင်း အတွက် စိတ်ကြိုက်ပြင်ဆင်ရန် ---
MY_TIKTOK_URL = "https://www.tiktok.com/@zayyarlin"

# ဘာသာစကား ဒေတာများ (Setting အဟောင်းများ အကုန်ပါသည်)
LANGUAGES = {
    'my': {'title': 'TikTok ဒေါင်းလုဒ်လုပ်သူ', 'placeholder': 'လင့်ခ်ကို ဤနေရာတွင် ထည့်ပါ', 'btn': 'ဗီဒီယိုရယူမည်', 'how': 'အသုံးပြုနည်း', 'safety': 'လုံခြုံမှုရှိပါသည်', 'about': 'Tool အကြောင်း'},
...
<style>
    ...
    .nav-bottom { 
        ...
        display: flex; justify-content: space-around; padding: 12px 0; border-top: 1px solid #333; z-index: 1000;
    }
    ...
    .step-box { background: rgba(0,242,234,0.1); border-radius: 15px; padding: 15px; margin-bottom: 15px; border-left: 5px solid var(--cyan); }
</style>
"""
...
def home():
    ...
    t = LANGUAGES.get(lang, LANGUAGES['my'])
    video_url = None
    
    if request.method == "POST":
        tt_url = request.form.get("tiktok_url")
        if tt_url:
            api_url = f"https://www.tikwm.com/api/?url={tt_url}"
            try:
    ...
    <html><head><meta name='viewport' content='width=device-width, initial-scale=1'>{STYLE}</head><body class='{mode}'>
        <div class='container p-4 text-center'>
            <h2 class='fw-bold mb-4 animate__animated animate__bounceIn'>
                <i class='fab fa-tiktok text-danger'></i> {t['title']}
            </h2>

            <div class='glass-card animate__animated animate__fadeInUp'>
                <form method='POST'>
                    <input type='text' name='tiktok_url' placeholder='{t['placeholder']}' class='form-control rounded-pill mb-3 py-3 border-0 bg-secondary bg-opacity-10 text-white' required>
                    <button class='btn btn-main w-100 py-3'>{t['btn']}</button>
                </form>
                {{% if video_url %}}
                    <a href='{{{{video_url}}}}' target='_blank' class='btn btn-success rounded-pill mt-3 w-100 py-3 fw-bold'>
            ...
            <div class='row mt-5 animate__animated animate__fadeIn'>
                <div class='col-6 col-md-4 feature-box'>
                    <i class='fas fa-user-ninja info-icon text-warning'></i>
                    <h6 class='small fw-bold'>Developer ဇွဲ</h6>
                    <p style='font-size: 10px;' class='text-secondary'>ဇေယျာလင်း ကိုယ်တိုင် အပင်ပန်းခံ ရေးသားထားပါသည်။</p>
                </div>
                <div class='col-6 col-md-4 feature-box'>
                    <i class='fas fa-shield-alt info-icon text-info'></i>
                    <h6 class='small fw-bold'>၁၀၀% လုံခြုံမှု</h6>
                    <p style='font-size: 10px;' class='text-secondary'>ဒေတာများကို လုံးဝမသိမ်းဆည်းဘဲ Safe ဖြစ်စေရမည်။</p>
                </div>
            </div>

            <div class='mt-5 text-start' style='margin-bottom: 100px;'>
                <h5 class='fw-bold mb-3'><i class='fas fa-magic text-info'></i> {t['how']}</h5>
                <div class='step-box'>
                    <h6>၁။ လင့်ခ်ကို ကူးယူပါ</h6>
                    <p class='small mb-0'>TikTok ထဲမှ Share ခလုတ်ကိုနှိပ်ပြီး Copy Link ယူပါ။</p>
                </div>
                <div class='step-box'>
                    <h6>၂။ ဤနေရာတွင် ထည့်ပါ</h6>
                    <p class='small mb-0'>ကူးလာသောလင့်ခ်ကို အပေါ်ကကွက်လပ်တွင် Paste လုပ်ပါ။</p>
                </div>
                <div class='step-box'>
                    <h6>၃။ ဒေါင်းလုဒ်လုပ်ပါ</h6>
                    <p class='small mb-0'>ဗီဒီယိုရယူမည်ကိုနှိပ်ပြီး Download ဆွဲပါ။</p>
                </div>
            </div>
        </div>
