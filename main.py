...
app = Flask(__name__)
# Vercel ပေါ်မှာ Session အတွက် လိုအပ်သည်
...
def home():
    ...
    if request.method == "POST":
        ...
        if tt_url:
            # TikWM API သည် ပိုမိုတည်ငြိမ်သည်
            api_url = f"https://www.tikwm.com/api/?url={tt_url}"
            try:
                res = requests.get(api_url).json()
                # TikWM ၏ response structure အတိုင်း ပြင်ဆင်ထားသည်
                if res.get("code") == 0:
    ...
    <html><head><meta name='viewport' content='width=device-width, initial-scale=1'>{STYLE}</head><body class='{mode}'>
        <div class='container p-4 text-center'>
            ...
            <div class='glass-card'>
                ...
                {{% if video_url %}}
                    <div class='mt-3'>
                        <p class='text-success small'><i class='fas fa-check-circle'></i> Video Ready!</p>
                        <a href='{{{{video_url}}}}' target='_blank' class='btn btn-success rounded-pill w-100 py-3 fw-bold'>
                            <i class='fas fa-download'></i> Download Without Watermark
                        </a>
                    </div>
                {{% endif %}}
            </div>
            <div class='mt-5 text-start' style='margin-bottom: 80px;'>
